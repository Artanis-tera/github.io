...
function highlightLigand(a,b,c){$(".ligandContact").removeClass("ligandContact");for(var d={},e=0;e<b.length;e++)b[e]&&"_"!=b[e].charAt(0)&&(res=b[e].substring(1),chain=b[e].charAt(0),d[chain]||(d[chain]=[]),d[chain].push(res));var f=$(".alignTbl tr[struc_id="+c+"]"),g=f[0].hasAttribute("keyseqchain")?"keyseqchain":"chain",h;for(h in d){var k=f.filter("["+g+"="+h+"]").find("span").toArray();for(e=0;e<d[h].length;e++)for(var r=0;r<k.length;r++)if(k[r].getAttribute("s")==d[h][e]){k[r].className="ligandContact";
...
function highlightResiduesNGL(a,b,c,d){var e="";if(b){var f=getNGLRepresentationByName(c+".hover");if(f){d&&(e=f.repr.selection.string+" ",e.startsWith("none")&&(e=e.substring(5)));var g=getNGLComponentByName(c).structure;c="";for(d=0;d<a.length;d++)if(b.length)for(var h=0;h<b.length;h++)b[h][0]==b[h][1]?isResidueInStructureNGL(g,a[d],b[h][0])&&(c+=b[h][0]+":"+a[d]+" "):(console.warn("Not checking if the selection is part of the structure"),c+=b[h][0]+"-"+b[h][1]+":"+a[d]+" ");else isResidueInStructureNGL(g,
a[d],b)&&(c+=b+":"+a[d]+" ");new_selection=e+c;""==new_selection&&(new_selection="none");f.setSelection(new_selection);return new_selection}}else{e=getNGLComponentByName(c);c="";if(e){if(1==a.length)c=(b?b:"")+":"+a;else for(d=0;d<a.length;d++)c+=(b?b:"")+":"+a[d]+" ";e.autoView(c,1E3)}return c}}
function centerResRangeNGL(a,b,c,d,e,f){for(var g="",h=getNGLComponentByName(d).structure,k=0;k<a.length;k++)for(var r=f?b:c,m=b;m<=r;m++)e&&m!=b&&m!=r||isResidueInStructureNGL(h,a[k],m)&&(g+=m+":"+a[k]+" ");if(f)for(k=0;k<f.length;k++)isResidueInStructureNGL(h,f[k],c)&&(g+=c+":"+f[k]+" ");""!=g&&getNGLComponentByName(d).autoView(g,1E3)}
...
[0,1,0];1<=dot(c.axis,e)&&(e=[0,0,1]);plane_x=normalize(cross(c.axis,e));plane_y=normalize(cross(c.axis,plane_x));e=[];var f=[],g=[];"pv"==d?pv_viewer.get(a).structure().eachAtom(function(t){"CA"==t.name()&&g.push(t.pos())}):"ngl"==d&&getNGLComponentByName(a).structure.eachResidue(function(t){t.eachAtom(function(v){"CA"==v.atomname&&g.push([v.x,v.y,v.z])})});for(var h=[],k="",r=1,m=c.radius/2+24,n=-m;n<m;n+=1.5)for(var p=-m;p<m;p+=1.5)if(pos=[c.plane_one_center[0]+1.5*n*plane_x[0]+1.5*p*plane_y[0],
c.plane_one_center[1]+1.5*n*plane_x[1]+1.5*p*plane_y[1],c.plane_one_center[2]+1.5*n*plane_x[2]+1.5*p*plane_y[2]],!(distance(pos,c.plane_one_center)>c.radius+8)){pos2=[c.plane_two_center[0]+1.5*n*plane_x[0]+1.5*p*plane_y[0],c.plane_two_center[1]+1.5*n*plane_x[1]+1.5*p*plane_y[1],c.plane_two_center[2]+1.5*n*plane_x[2]+1.5*p*plane_y[2]];for(var q=0;q<g.length;)5>distance(pos,g[q])?e.push(g.splice(q,1)[0]):5>distance(pos2,g[q])?f.push(g.splice(q,1)[0]):q++;h.push([pos,pos2])}for(m=0;m<h.length;m++){n=
e;p=0;do{q=!1;for(var u=h[m][p++],l=0;l<n.length;l++)if(5>distance(u,n[l])){q=!0;break}q||(k+="ATOM  "+("     "+r++).slice(-5)+"   -  MEM M   1    "+("        "+u[0].toFixed(3)).slice(-8)+("        "+u[1].toFixed(3)).slice(-8)+("        "+u[2].toFixed(3)).slice(-8)+"  1.00  0.00    \n");n=f}while(2>p)}k+="END";if("pv"==d)pv_viewer.points(a+".membrane",pv.io.pdb(k),{color:pv.color.uniform("#8b8b8b"),pointSize:2}),c.viewTransform&&pv_viewer.setRotation(c.viewTransform);else if("ngl"==d){for(schemeId in NGL.ColormakerRegistry.userSchemes)-1<
...
PLIP.Bond=function(a,b,c,d,e,f,g,h,k){var r=!1,m=!1,n=void 0,p="LigandBond"+a,q=void 0,u="LigandBondHighlight"+a;this.show=function(l){null==l&&(l=!0);this.isVisible()?console.info("Bond already visible"):PLIP.AbstractViewer.isStructureEnabled(this.strucId)&&(this.draw(),void 0!==this.residue&&this.residue.show(!1),this.ligand.show(l),r=!0,l&&pv_viewer.requestRedraw())};this.hide=function(l){null==l&&(l=!0);PLIP.AbstractViewer.isStructureEnabled(this.strucId)&&this.isVisible()&&(this.destroy(),r=
...
PLIP.AbstractViewer.addWaterCircle(this.getBondMesh(),this.coo3,2*plip_bond_radius,this.color,!0))};this.destroy=function(){n=PLIP.AbstractViewer.removeMesh(p)};this.refresh=function(){this.destroy();this.isVisible()&&this.draw();this.destroyHighlight();this.isHighlighted()&&this.drawHighlight()};this.toggle=function(){this.isVisible()?this.hide():this.show()};this.isVisible=function(){return r};this.getBondMesh=function(){void 0==n&&(n=PLIP.AbstractViewer.newBondMesh(p));return n};this.highlight=
