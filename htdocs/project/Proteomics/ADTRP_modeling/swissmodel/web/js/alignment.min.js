...
{}),e[f].selected){if(!(a in H[d.$jscomp$loop$prop$identifier$7$26]))if(-1<a.indexOf("bfactor")){H[d.$jscomp$loop$prop$identifier$7$26].bfactor={};d.$jscomp$loop$prop$assigned$25=!1;if(0<g("#pv:visible",currentWin.document).length)pv_viewer.forEach(function(t){return function(q){if(q.name()==t.$jscomp$loop$prop$identifier$7$26){for(var r={},v=0;v<q.structure().chains().length;r={$jscomp$loop$prop$cname$9$22:r.$jscomp$loop$prop$cname$9$22},v++){var w=q.structure().chains()[v];r.$jscomp$loop$prop$cname$9$22=
...
null;H[d.$jscomp$loop$prop$identifier$7$26].bfactor_range=null;continue}if("bfactor_range"==a){H[d.$jscomp$loop$prop$identifier$7$26].bfactor_range={};k=[];for(l=0;256>l;l++)k.push(l+","+l+",255");for(l=256;512>l;l++)k.push("255,"+(512-l)+","+(512-l));rms/=count;rms=Math.sqrt(rms);mx=2*rms;for(chn in H[d.$jscomp$loop$prop$identifier$7$26].bfactor)for(H[d.$jscomp$loop$prop$identifier$7$26].bfactor_range[chn]={colours:[],values:[]},l=0;l<H[d.$jscomp$loop$prop$identifier$7$26].bfactor[chn].values.length;l++)(m=
...
return}}else if("undefined"!=typeof SCORE_URL){H[d.$jscomp$loop$prop$identifier$7$26][a]=null;g.getJSON(SCORE_URL.replace("xxxx.0",d.$jscomp$loop$prop$identifier$7$26)+a,function(t){return function(q,r){r&&(H[t.$jscomp$loop$prop$identifier$7$26][a]=q,g(".alignTbl").each(function(){Ka(g(this),a)}),qa())}}(d));return}if(null==H[d.$jscomp$loop$prop$identifier$7$26][a])return}c.find("tr[struc_id]").each(function(){var t=g(this).attr("chain");t||(t=g(this).attr("keyseqchain"));if(t){identifier=-1==a.indexOf("bfactor")?
g(this).attr("qname").match(/\w+\.\d+/)[0]:g(this).attr("struc_id");if(null==H[identifier][a])return!0;if(null==H[identifier][a][t])for(var q in H[identifier][a])for(var r=0;r<q.length;r++)q[r]==t&&(H[identifier][a][q[r]]=H[identifier][a][q]);if(H[identifier]&&H[identifier][a]&&H[identifier][a][t]){var v=null;g(this).find("span[s]").each(function(w,u){v=u.getAttribute("s");null!=H[identifier][a][t].values[v-1]&&(u.title=R[g(this).text()].triplet+" : "+v+"; "+a+" : "+H[identifier][a][t].values[v-1].toFixed(2));
u.style.backgroundColor="rgba("+H[identifier][a][t].colours[v-1]+",.6)"})}}})}}}function qa(c){0<g("#pv:visible",currentWin.document).length&&cb();0<g("#ngl:visible",currentWin.document).length&&(c&&c.singleResidue?db(c.singleResidue):eb(c));if(!("function"!==typeof colourUPArcs&&"function"!==typeof colourRama||c&&c.singleResidue)){var a={};g(".alignTbl").each(function(){$alignTbl=g(this);var e=$alignTbl.data("templateData");if(e&&!$alignTbl.data("options").textOnly){var d=-1,f=null;"model-template"==
e.type?$rows=$alignTbl.find("tr[keyseqchain]"):($rows=$alignTbl.find("tr[chain]"),f=$alignTbl.find("tr[seqno=1]").find("span"));for(;;){d++;e=$rows.filter("[seqno="+d+"]");if(1>e.length)if(1<d)break;else continue;var k=e.attr("chain");k||(k=e.attr("keyseqchain"));var l=e.attr("struc_id"),m=[];a[l]||(a[l]={});a[l][k]||(a[l][k]=m,e.find("span").each(function(n,t){var q=g(t).css("background-color");q&&"transparent"!=q&&-1==q.indexOf("(0, 0, 0")&&(q=q.match(/\d+, \d+, \d+/)[0].split(","),m[f?f[n].getAttribute("s"):
t.getAttribute("s")]=q)}))}}});"function"===typeof colourRama&&colourRama(a);"function"===typeof colourUPArcs&&colourUPArcs(a)}}function db(c){for(schemeId in NGL.ColormakerRegistry.userSchemes)-1<schemeId.indexOf("singleres")&&(NGL.ColormakerRegistry.removeScheme(schemeId),ngl_stage.eachRepresentation(function(e){if(e.repr.colorScheme==schemeId)for(schemeId2 in NGL.ColormakerRegistry.userSchemes)if(schemeId2.endsWith(e.name)){e.setColor(schemeId2);break}}));if(c){var a=c.struc_id;getNGLComponentByName(a)&&
ngl_stage.eachRepresentation(function(e){if(e.name==a&&U[a]){var d=U[a],f=NGL.ColormakerRegistry.addScheme(function(k){for(chn in d)" "==chn&&g("#ngl_status").html("NGL cannot colour or select chains named whitespace!");this.atomColor=function(l){return d[l.chainname]?ngl_picked_atom&&l.structure.name==ngl_picked_atom.structure.name&&l.index==ngl_picked_atom.index?16711680:(l=d[l.chainname][l.resno])?l[0]<<16|l[1]<<8|l[2]:16777215:16777215}},"singleres");e.setColor(f)}})}}function eb(c){var a=null,
e;U={};g(".alignTbl:not(.ignoreStructure)").each(function(){e=g(this);var k=e.data("templateData");if(k&&!e.data("options").textOnly){var l=-1;for(a="model-template"==k.type?e.find("tr[keyseqchain]"):e.find("tr[chain]");;){l++;var m=a.filter("[seqno="+l+"]");if(1>m.length)if(1<l)break;else continue;(k=m.attr("chain"))||(k=m.attr("keyseqchain"));var n=m.attr("struc_id");m=m.find("span");var t=null;t=U[n]&&U[n][k]?U[n][k]:[];m.each(function(q,r){var v=g(r).css("background-color");v&&"transparent"!=
v&&-1==v.indexOf("(0, 0, 0")&&(v=v.match(/\d+, \d+, \d+/)[0].split(","),t[r.getAttribute("s")]=v)});U[n]||(U[n]={});U[n][k]=t}}});c=c&&c.strucId?c.strucId:null;for(var d in ngl_stage.compList)if(void 0!==ngl_stage.compList[d].structure){var f=ngl_stage.compList[d].name;if(!c||f==c){for(schemeId in NGL.ColormakerRegistry.userSchemes)-1<schemeId.indexOf(f)&&-1==schemeId.indexOf("membrane")&&NGL.ColormakerRegistry.removeScheme(schemeId);ngl_stage.eachRepresentation(function(k){if((k.name==f||k.name.endsWith(".highlight"))&&
U[f]){var l=U[f],m=NGL.ColormakerRegistry.addScheme(function(n){for(chn in l)" "==chn&&g("#ngl_status").html("NGL cannot colour or select chains named whitespace!");this.atomColor=function(t){return l[t.chainname]?(t=l[t.chainname][t.resno])?t[0]<<16|t[1]<<8|t[2]:16777215:16777215}},f);k.setColor(m)}})}}}function cb(){if(0!=pv_viewer.all().length){var c=null,a,e={};g(".alignTbl:not(.ignoreStructure)").each(function(){a=g(this);var k=a.data("templateData");if(k&&!a.data("options").textOnly){var l=
-1;for(c="model-template"==k.type?a.find("tr[keyseqchain]"):a.find("tr[chain]");;){l++;var m=c.filter("[seqno="+l+"]");if(1>m.length)if(1<l)break;else continue;(k=m.attr("chain"))||(k=m.attr("keyseqchain"));var n=m.attr("struc_id");m=m.find("span");var t=null;t=e[n]&&e[n][k]?e[n][k]:[];m.each(function(q,r){var v=g(r).css("background-color");v&&"transparent"!=v&&-1==v.indexOf("(0, 0, 0")&&(v=v.match(/\d+, \d+, \d+/)[0].split(","),t[r.getAttribute("s")]=v)});e[n]||(e[n]={});e[n][k]=t}}});for(var d in e){var f=
e[d];pv_viewer.get(d)&&pv_viewer.get(d).colorBy(new pv.color.ColorOp(function(k,l,m){k=k.residue();f[k.chain().name()]&&((k=f[k.chain().name()][k.num()])?(l[m]=k[0]/255,l[m+1]=k[1]/255,l[m+2]=k[2]/255):(l[m]=.9,l[m+1]=.9,l[m+2]=.9),l[m+3]=1)}))}d&&pv_viewer.requestRedraw()}}function ia(c){var a=g.cookie("secStruct");null==a&&(a="dssp");var e=c.attr("id")+"_features";g("#"+e).html("");if(0==g("#"+e).length||a&&"none"==a)c.removeClass("ss"),G(c).removeClass("ss");else{c.addClass("ss");G(c).addClass("ss");
var d=c.position().top,f=c.find("td").first().width(),k=c.find("span")[0],l=k.getBoundingClientRect().width;k=k.getBoundingClientRect().height;var m=c.position().left;if(!(1>f)){p=Raphael(e,f,c.height());g("#"+e).css("top",c.position().top);var n=c.data("templateData");if(n){c=c.find("tr:visible");if(n.keyseq_annotation)for(e=0;e<n.keyseq_chain_name.length;e++){var t=n.keyseq_chain_name[e];if(n.keyseq_annotation[t]){var q=n.templates[0].trg_offset?n.templates[0].trg_offset:1;n.keyseq_annotation[t][a]&&
Qa(p,Ra(n.keyseq_annotation[t],a,q),c.filter('[keyseqchain="'+t+'"]'),c.filter('[keyseqchain="'+t+'"]').find("span[s]").toArray(),d,m,f,l,k);n.charts&&function(A,D,B){setTimeout(function(){fb(A,n.keyseq_annotation[D],a,q,B,d,f,l,5E3>n.pageResidueCount)},0)}(p,t,c.filter('[keyseqchain="'+t+'"]'))}}e=0;for(t=1;t<c.length;t++){var r=c.eq(t);if(parseInt(r.attr("seqno"))<e)break;e=parseInt(r.attr("seqno"));if(!r.attr("keyseqtype")){var v=r.attr("tpl_id");r=r.attr("chain");$row=v?c.filter('[tpl_id="'+v+
'"]'):c.filter('[chain="'+r+'"]');for(var w=$row.find("span[s]").toArray(),u=0;u<n.templates.length;u++)(v?n.templates[u].id==v:n.templates[u].chain==r)&&n.templates[u].annotation&&n.templates[u].annotation[a]&&Qa(p,Ra(n.templates[u].annotation,a),$row,w,d,m,f,l,k)}}}}}}function Oa(c,a){var e=c[a];"string"==typeof e&&(c[a+"_string"]=e.replace(/[GI]/g,"H").replace(/B/g,"E"));return c[a+"_string"]}function Ra(c,a,e){e||(e=1);var d=c[a];if("string"==typeof d){c[a+"_string"]=c[a];d=c[a].replace(/[GI]/g,
"H").replace(/B/g,"E");for(var f=null,k=0;k<d.length;k++)if("H"==d[k]||"E"==d[k])res_index=k+e,null==f?f=[[res_index,res_index,d[k]]]:f[f.length-1][1]==res_index-1&&f[f.length-1][2]==d[k]?f[f.length-1][1]=res_index:f.push([res_index,res_index,d[k]]);c[a]=f}return c[a]}function fb(c,a,e,d,f,k,l,m,n){var t=null!=a.qmean;d||(d=1);var q=c.set(),r=c.set(),v=c.set();f.each(function(w,u){span=g(u).find("span").toArray();if(0!=span.length){var A=ea-W,D=g(u).position().top-k+g(span[0]).position().top-W,B=
D,C=u.getBoundingClientRect().left,F=span[span.length-1].getBoundingClientRect().right-C;if(t){c.path("M0,"+B+"L"+l+","+B+"L"+l+","+(B-A)+"L0,"+(B-A)).attr({"stroke-width":.4,stroke:"#888"});for(var K=3;10>K;K++){var M=B-1/.7*A*(K/10-.3);var I=c.path("M0,"+M+"L"+l+","+M);5==K?I.attr({"stroke-width":.7,opacity:.8,stroke:"#888","stroke-dasharray":"-"}):q.push(I)}}I="";var O;for(K=0;K<span.length;K++)if(M=span[K],M.hasAttribute("s")){if(B=parseInt(M.getAttribute("s"))-d,I=a[e+"_string"]&&a[e+"_string"][B]?
I+a[e+"_string"][B]:I+"-",x=M.getBoundingClientRect().left-C,B=D,B-=2,t){var X=Math.max(.3,parseFloat(M.getAttribute("q")));M=B-1/.7*(A-2)*(X-.3);if(n){var J="245,80,0";.5<X&&(w=Math.round(490*(1-X)),b=Math.round(255*X),J=w+",80,"+b);s=c.path("M"+x+","+B+"L"+x+","+M+"L"+(x+m)+","+M+"L"+(x+m)+","+B);s.attr({fill:"rgba("+J+",.6)",stroke:"#333"})}else O+="M"+x+" "+B,O+="L"+x+" "+M,O+="L"+(x+m)+" "+M,O+="L"+(x+m)+" "+B}}else I+="-";txt=c.text(2,D+W/2,I);r.push(txt);txt.node.setAttribute("textLength",
F-4+"px");txt.node.firstChild.setAttribute("textLength",F-4+"px");v.push(c.path(O))}});r.attr({"text-anchor":"start","font-family":"consolas"});v.attr({stroke:"#555"});q.attr({"stroke-width":.9,stroke:"#888",opacity:.6,"stroke-dasharray":". "})}function Qa(c,a,e,d,f,k,l,m,n){if(null!=a){e.attr("seqno");for(var t=0,q=d.length,r=0;r<a.length;r++)if("E"==a[r][2]||"H"==a[r][2]){var v=null,w=null,u;for(u=a[r][0];u<=a[r][1];u++)if(!(d[t].getAttribute("s")>u)){for(s=t;s<q;s++)if(d[s].getAttribute("s")==
u){t=s;v=g(d[s]);break}if(v)break}for(u=a[r][1];u>=a[r][0];u--){for(s=t;s<q;s++)if(d[s].getAttribute("s")==u){t=s;w=g(d[s]);break}if(w)break}if(null==v||null==w)t=0;else for(thisRow=v.closest("tr"),startRowIdx=e.index(thisRow),endRowIdx=e.index(w.closest("tr")),v=v.position(),startResLeft=v.left-k,spanTop=v.top,endResLeft=w.position().left-k,rightLimit=thisRow.width()-m-m,w=startRowIdx;w<endRowIdx+1;w++)if(w>startRowIdx&&(thisRow=e.eq(w),spanTop=thisRow.find("span").first().position().top),u=thisRow.position().top+
spanTop-f,"E"==a[r][2]){v=c;var A=startResLeft-(w-startRowIdx)*l,D=endResLeft+(endRowIdx-w)*l,B=m,C=n,F=rightLimit;u+=1;C-=2;padding=2;A=Math.max(-B,Math.round(A));D=Math.min(F,Math.round(D+B));s=v.path("M"+A+","+(u-padding)+"L"+(D-6)+","+(1.5+u)+"L"+(D-6)+","+(u-padding)+"L"+(D+padding)+","+(C/2+u)+"L"+(D-6)+","+(C+u+padding)+"L"+(D-6)+","+(C-1.5+u)+"L"+A+","+(C+u+padding)+"L"+A+","+(u-padding));s.attr({"stroke-width":.7,fill:"#DDD","fill-opacity":.3,stroke:"#226","stroke-dasharray":"5 2"})}else v=
c,A=startResLeft-(w-startRowIdx)*l,D=endResLeft+(endRowIdx-w)*l,B=m,C=n,F=rightLimit,padding=2,A=Math.max(-B,Math.round(A)),D=Math.min(F,Math.round(D+B)),h=v.rect(A,u+2-padding,D-A,C-4+2*padding,4),h.attr({"stroke-width":.7,fill:"#DDD","fill-opacity":.3,stroke:"#226"})}}}function Sa(){var c="";"Multi FASTA"==g("#alignmentFormatSelect").val()?g(".alignTbl:visible").each(function(){c+=Ba(g(this).attr("id").substring(8),"FASTA");c+="\n"}):c=Ba(g("#currentAlignmentIndex").val(),g("#alignmentFormatSelect").val());
g("#formattedAlignmentTextarea").val(c);g("#formattedAlignmentTextarea").focus();g("#formattedAlignmentTextarea").select()}function Ba(c,a){a||(a="fasta");var e=g("#alignNameTbl"+c),d=g("#alignTbl"+c),f=[],k=[],l=[],m=0<g("#alignTbl"+c+" :visible").first().length?":visible":"",n=0;e.find("tr[seqno]"+m).each(function(t,q){parseInt(q.getAttribute("seqno"))>n&&n++}).slice(0,n).each(function(t){var q=g(this).attr("seqno");f[t]=g(this).text();l[t]=g(this).attr("title");breakCount=0;d.find('tr[seqno="'+
q+'"]').each(function(r){k[t]||(k[t]="");k[t]+=g(this).find("td").first().text();breakCount+=g(this).find(".insert_l,.insert_r").length});0<breakCount&&(f[t]="MULTIPLE_SEGMENTS_OF_"+f[t]+" - WARNING - CONTAINS NON-SEQUENTIAL SEGMENTS")});return-1<a.search(/fasta/i)?gb(f,k,l):-1<a.search(/clustal/i)?hb(f,k):""}function gb(c,a,e){if(!a||0==a.length)return"No templates selected";for(var d="",f=0;f<c.length;f++){d+=">"+c[f]+(e&&e[f]?" "+e[f]:"")+"\n";for(var k=0;k<a[f].length;k+=72)d+=a[f].substring(k,
k+72)+"\n"}return d}function hb(c,a){if(!a||0==a.length)return"No templates selected";for(var e=0,d=0;d<c.length;d++)e=Math.max(e,c[d].length);e+=2;for(var f="CLUSTAL\n\n",k=0;k<a[0].length;k+=60){for(d=0;d<c.length;d++){f+=c[d];for(var l=c[d].length;l<e;l++)f+=" ";f+=a[d].substring(k,k+60)+"\n"}f+="\n"}return f=f.substring(0,f.length-2)}function ib(c){$alignNameTbl=G(c);var a=c.closest(".alignTblContainer").outerWidth(),e=c.closest(".alignTblContainer").outerHeight();a+=10;var d='<svg xmlns="http://www.w3.org/2000/svg"  width="'+
a+'" height="'+e+'"><rect x="0" y="0" fill="white" width="'+a+'" height="'+e+'"/>',f,k=$alignNameTbl.outerWidth()+10,l=$alignNameTbl.offset().left-10,m=c.find("td:eq(0)");a=m.outerWidth();var n=g("<div>").append(g("#"+c.attr("id")+"_features svg").first().clone()).html().replace(/name="[\d_]+" /g,"").replace(/r="4" /g,"");d+='<defs><clipPath id="cp2"><rect x="0" y="0" width="'+a+'" height="'+e+'"/></clipPath></defs>';d+='<g transform="translate('+k+',0)" clip-path="url(#cp2)">';d+=n.substring(n.indexOf("</defs>")+
7,n.indexOf("</svg>"));d+="</g>";e=c.find("tr:visible");e.each(function(r,v){g(this).find("span.ligandContact").each(function(w,u){f=v.offsetTop+u.offsetTop;t||(t=g(u).outerWidth(),q=g(u).outerHeight());g(u).css("color");d+='<rect style="fill:#CC4444;fill-opacity:0.8;" width="'+t+'" height="'+q+'" x="'+(g(u).offset().left-l)+'" y="'+f+'" />'})});d+='<text text-anchor="end" font-size="'+m.css("font-size")+"\" font-family='"+m.css("font-family")+"'>";e=$alignNameTbl.find("tr:visible");e.each(function(r,
v){var w=g(this).find("td").css("padding-top");w=parseInt(w.substring(0,w.length-2));f=v.offsetTop+12+w;d+='<tspan x="'+(k-2)+'" y="'+f+'">'+v.textContent.replace("QMEAN","")+"</tspan>"});d+="</text>";e=c.find("tr:visible");var t,q;e.offset();e.each(function(r,v){d+='<text text-anchor="middle" font-size="'+m.css("font-size")+"\" font-family='"+m.css("font-family")+"'>";g(this).find("span").each(function(w,u){t||(t=g(u).outerWidth(),q=g(u).outerHeight());if("relative"!=u.style.position){var A=g(u).css("color");
A="fill:"+A+";";"bold"==u.style.fontWeight&&(A+="font-weight:bold;");0<A.length&&(A=' style="'+A+'" ');f=v.offsetTop+u.offsetTop+12;d+='<tspan x="'+(g(u).offset().left-l+t/2)+'" y="'+f+'"'+A+">"+u.textContent+"</tspan>"}});d+="</text>";(td=g(this).find("td")[1])&&(d+='<text text-anchor="start" font-size="'+m.css("font-size")+"\" font-family='"+m.css("font-family")+"'><tspan x=\""+(g(td).offset().left-l+10)+'" y="'+f+'">'+td.textContent+"</tspan></text>")});e.each(function(r,v){g(this).find("span").not(".ligandContact").each(function(w,
u){f=v.offsetTop+u.offsetTop;var A=g(u).css("backgroundColor");A&&"transparent"!=A&&"rgb(255, 255, 255)"!=A&&(d+='<rect x="'+(g(u).offset().left-l)+'" y="'+f+'" width="'+t+'" height="'+q+'" style="fill:'+A+';"/>')})});return d+"</svg>"}var H={},da={gapChar:"-"},Ta=!1,ha=!1,N=null,ea=0,W=0,ra=!1,va=null,V=0,fa=0,P="rgba(136,221,204,.6) rgba(213,178,64,.6) rgba(187,187,221,.6) rgba(255,136,119,.6) rgba(136,187,221,.6) rgba(255,187,102,.6) rgba(187,221,102,.6) rgba(255,204,238,.6) rgba(166,206,227,.6) rgba(187,136,187,.6) rgba(204,238,204,.6) rgba(166,206,227,.6)".split(" "),
na={init:function(c){Ta||(Ta=!0,g(document).keydown(function(a){if(ha&&1>g(".fancybox-desktop:visible").length)a:if(ra)a.preventDefault();else{var e=g("span.cursor").closest(".alignTbl");if(32==a.keyCode)a.preventDefault(),Da();else if(8==a.keyCode||46==a.keyCode)a.preventDefault(),Ca(8==a.keyCode);else if(27==a.keyCode){e.data("startEditRegion",null);e.data("endEditRegion",null);g(".shadowCursor").removeClass("shadowCursor");break a}else if((a.ctrlKey||a.metaKey)&&90==a.keyCode){a=g("body").data("editHistory");
